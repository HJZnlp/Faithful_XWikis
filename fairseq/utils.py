...
def parse_noise(line):
    """
    Parses a single line from the noisy labels file.

    Args:
        line (str): String containing the label noise information in the following format:
            entail_score ||| list of doc sentence indices (starting at 0) ||| binary_entail_label

    Returns:
        torch.IntTensor: binary_entail_label
    """
    
    scores=line.strip().split(" ")
    labels = torch.IntTensor(len(scores))
    for i,c in enumerate(scores):
        labels[i]=float(c)>0.1
    # if line.strip()=="":
    #     return torch.IntTensor([-1])
    # noisy_label = float(line.strip().split('|||')[0].strip())>0.1
    # if noisy_label==False:
