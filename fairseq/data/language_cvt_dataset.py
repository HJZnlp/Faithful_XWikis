...
class CvtDenoisingDataset(DenoisingDataset):
    def __init__(
        self,
        dataset,
        sizes,
        ...
        args,
        eos=None):
        super(CvtDenoisingDataset, self).__init__(dataset,
        sizes,
        vocab,
        mask_idx,
        mask_whole_words,
    ...
    def collater(self, samples):
        ...
        """
        return collate_deno(samples, self.vocab.pad(), self.eos, self.vocab, left_pad_target=True,)

    def size(self, index):
        """Return an example's size as a float or tuple. This value is used when
        filtering a dataset with ``--max-positions``."""
        return (self.sizes[index], 0) ## have same shape as expected for RoundRobin dataset
