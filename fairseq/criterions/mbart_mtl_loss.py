...
class MBARTMtlLoss(FairseqCriterion):
    ...
    def build_underlying_criterion(args, task):
        """ TODO: if all works... then improve this criterion building! read from config files"""
        ...
        #for aux_task in task._aux_src_classif_tasks:
        #    # current valid tasks are ['disc', 'srl', 'cat'], do better pass input parameters
    ...
    def build_criterion(cls, args, task):
        ...
        class _CompositeLoss(FairseqCriterion):
            ...
            def forward(self, model, sample, reduce=True):
                ...
                if task_type == 'mainsum':
                ...
                elif task_type == 'cvt' and self.args.cvt_p > 0:
                    ...
                    if 'few' in sample.keys() and sample['few'][0]:
                        ...
                        if self.args.cvt_future_pred:
                            ...
                            #print('cvt predictions', net_outputs[0])
                elif task_type == 'cvt':
                    net_outputs = model(**sample['net_input'])
                elif task_type =='selfsrc':
                    #forward without teacher forcing...UNIMPLEMENTED
                    net_outputs = model.forward_selfsrc(**sample['net_input'])
                ...
                for o, extr, t in zip(outputs, extras, targets):
                    ...
                    #sample['target'] = t
                    #l, ss, lo = self.underlying_criterions[task_type](m, sample, reduce)
                    #loss, ss, nll_lo, extra = self.underlying_criterions['mainsum'](m, sample, reduce)
                    ...
                    if task_type == 'cvt' and self.args.cvt_target_wei and not ('few' in sample.keys() and sample['few'][0]):
                        ...
                        #print(sample['target_score']) #hacky I know is dim 1
                        #exit()
                    ...
                    if task_type == 'cvt' and not self.args.updates_per_task \
                        ...
                        l = l *  self.args.cvt_proportion #weight when it's unlabelled data
                        #print("Some cvt", utils.item(l.data))

                    if task_type == 'cvt' and not self.args.updates_per_task \
                                            and self.args.few_proportion>0 \
                                            and ('few' in sample.keys() and sample['few'][0]):
                        l = l *  self.args.few_proportion #weight when it's unlabelled data
                        #print("*******************************Some few", utils.item(l.data))

                    if task_type != 'cvt' and self.args.mono_proportion > 0 :
                        l = l * self.args.mono_proportion
                        #print("Some mono", utils.item(l.data))
