...
class CandidatePenaltyCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, compute_custom_metrics=True):
        ...
        # TODO: need to see here we'll compute mle only for non-noisy labels
        mle_loss = true_token_lprobs.sum()

        # TODO: revise/implement the following code from the unlikelihood paper: https://arxiv.org/abs/1908.04319
        # TODO: we do not need to analyse candidate tokens, we need to take the noisy labels annotations
        # TODO: comming in batch/sample 'noise_labels'. Need to compute unlikelihood loss on these cases.
        # TODO: code about metrics below may need to be removed, we'll may not use it
        # -- custom loss
