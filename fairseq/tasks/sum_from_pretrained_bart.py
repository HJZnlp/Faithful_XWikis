...
def load_langpair_dataset(
    ...
    max_target_positions, prepend_bos=False, load_alignments=False, load_noise_labels=False,noise_path=None,
    ...
    noise_dataset = None
    if load_noise_labels:
        if noise_path is not None:
            noise_root = os.path.join(noise_path, '{}.noise.{}-{}'.format(split, src, tgt))
            if indexed_dataset.dataset_exists(noise_root, impl=dataset_impl):
                noise_dataset = data_utils.load_indexed_dataset(noise_root, None, dataset_impl)
                # print(noise_dataset[0].data.tolist())
                # print(noise_dataset[1].data.tolist())
                # print(noise_dataset[2].data.tolist())
    ...
    tgt_dataset_sizes = tgt_dataset.sizes if tgt_dataset is not None else None
        ...
        # if 0 is not in label.data:

        # break
    # print(noise_dataset[0].data)
    # exit()
    # if split=="valid":
    #     count=0
    #     for c in noise_dataset:
    #         count+=1
    #         # print(i.data)
    #         print(c.data)
    #         if count>5:
...
class SumFromPretrainedBARTTask(TranslationFromPretrainedBARTTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--load-noise-labels', action='store_true',
                            help='load the binarized noise labels')
        parser.add_argument('--noise-path', type=str,default=None,
                            help='path to the binarized noise labels')
        parser.add_argument('--remove-stop-words', type=int,default=1,
                            help='remove stop word in masked sentence')
        # fmt: on
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_dataset(
            ...
            load_noise_labels=self.args.load_noise_labels,
            ...
            remove=self.args.remove_stop_words,
            )
            # exit()
