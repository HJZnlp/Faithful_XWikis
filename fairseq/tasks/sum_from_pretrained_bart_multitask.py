...
def load_langpair_dataset(
    ...
    if args.cvt_few > 0 :
        ...
        np.random.seed(args.cvt_seed) #args.seed, 44, 1, 96
        indices = np.random.choice(target_idxs, actual_size, replace=False)
        np.random.seed(args.seed) # reset the general seed
...
class SumFromPretrainedBARTMultiTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--cvt-layers', default='', type=str,
        ...
        parser.add_argument('--cvt-seed', default=None, type=int, metavar='N',
                            help='pseudo random number generator seed for taking different samples for few/cvt')
